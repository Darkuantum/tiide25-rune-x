...
## Problem
The application was failing to make Gemini API calls because Node.js native `fetch()` doesn't support SOCKS5 proxies. Even though the endpoints are reachable through the proxy (as confirmed by the connectivity test), the application code couldn't use them.

## Solution
Modified `src/lib/ai-processor.ts` to use Node's `https` module with SOCKS5 proxy support instead of `fetch()` for Gemini API calls.
